...
for i in range(K-1):
    ...
    regr = DecisionTreeRegressor(max_depth=20, max_leaf_nodes=50)
    ...
    huber_err[0][i] = huber(y_train, y_pred_train, 0.1)/ y_pred_valid.size
    ...
    logcosh_err[0][i] = logcosh(y_train, y_pred_train)/ y_pred_valid.size
