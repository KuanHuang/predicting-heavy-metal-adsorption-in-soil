...
for i in range(K-1):
    ...
    regr = xgb.XGBRegressor(learning_rate=0.1, max_depth=6, n_estimators=50, min_child_weight=4)
...
np.savetxt("xgb_pred_train.csv", sheet_data_train, delimiter=",")
...
np.savetxt("xgb_pred_valid.csv", sheet_data_valid, delimiter=",")
...
np.savetxt("xgb_pred_test.csv", sheet_data_test, delimiter=",")
